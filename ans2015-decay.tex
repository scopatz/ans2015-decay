...
($\lambda_C$) goes to zero.  Also, notice in Equation \ref{c_i} that every $c_i$ contains exactly one $\lambda_C$
...
\label{bin}
There are two main strategies that may be used to construct a version of these 
...
This can be done by grouping constants together and pre-calculating the single 
resulting constant. This saves the 
computer from having to perform these same arithmetic operations at run time.  
Using these principles it is possible to express the Bateman equations as a simple sum of 
...
because explicitly computing $c_i$ has been completely avoided.
...
powers of 2 than any other base, even the natural base $e$. Thus the 
